...
static volatile uint8_t freq_flag = 0; //if 0 use function generator, if 1 use 555
...
int main(int argc, char *argv[]) {
	...
	while (1) {
		//trace_printf("btn pressed freq flag: %u \n", freq_source_flag);
		//Start ADC
...
void refresh_OLED(void) {
	...
	oled_DrawStrings(0, 0, Buffer);
		...
		snprintf(Buffer, sizeof(Buffer), "F: %8u Hz", Freq);
			/* Buffer now contains your character ASCII codes for LED Display
			 - select PAGE (LED Display line) and set starting SEG (column)
			 - for each c = ASCII code = Buffer[0], Buffer[1], ...,
			 send 8 bytes in Characters[c][0-7] to LED Display
	...
	if(freq_flag == 0){
		snprintf(Buffer, sizeof(Buffer), "Source: Func Gen");
	}
	else{
		snprintf(Buffer, sizeof(Buffer), "Source: N555    ");
	}
	oled_DrawStrings(3, 0, Buffer);
...
void btn_EXTI_Init()//external interrupt for button
{
...
void EXTI3_555_Init() {
	...
	// Map EXTI3 line to PB3: EXTICR[0] bits 11:8 = 0001 (Port B) */
	SYSCFG->EXTICR[0] |= (SYSCFG->EXTICR[0] & ~(0xF << 12)) | (0x1 << 12);
